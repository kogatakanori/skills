...
| Who | 少なくとも1つのユーザー種別が定義されているか。技術レベル・利用文脈が記述されているか |
...
ConstraintsはビジネスルールやクライアントFixの条件のため、テストで検証できない「条件・選定型」と、テスト可能な「非機能要件型」がある。

**条件・選定型**（計測不要、そのままでよい）:
- ✅ `TypeScriptで実装すること`
- ✅ `AWSのみ使用すること`
- ✅ `個人情報をサードパーティに送信しないこと`

**非機能要件型**（数値・基準値で書かれているかチェックする）:
- ❌ `高速に応答すること` → 何秒以内かが不明
- ❌ `多くのユーザーが利用できること` → 何人かが不明
- ✅ `3秒以内にレスポンスを返すこと`
- ✅ `同時100ユーザーの利用に耐えること`

非機能要件型のConstraintに具体的な数値がない場合のみ HIGH として報告する。
