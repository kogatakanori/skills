...
Specが前提としているデータ・状態・値（例：「このステータスはUIから設定される」「この値はAPIから常に渡ってくる」等）について、実際にUI/API等の入力経路を辿ってコードを読み、前提が実データ・実装と一致しているか確認してください。Specの記述を鵜呑みにせず、入力元まで遡って裏取りすることが目的です。矛盾や前提が崩れている箇所を見つけた場合は、報告に含めてください。
