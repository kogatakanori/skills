...
**Agent B（architecture-analyst）**: 起動条件はCHANGE_TYPEで分岐する（アーキテクチャ制約・既存docs・テスト基盤を調査）：
- 変革型: 常に起動
- 踏襲型: Specに「アーキテクチャ|レイヤー|ディレクトリ構成|パッケージ構成|新規モジュール|新規サービス」のいずれかを含む場合のみ起動

`[specの全文]` を置換して起動。

**Agent C（dependency-analyst）**: 起動条件はCHANGE_TYPEで分岐する（ライブラリ・外部APIの存在・バージョン適合性・破壊的変更リスクを確認）：
- 変革型: 常に起動
- 踏襲型: Specに「ライブラリ|パッケージ|SDK|外部API|依存関係|バージョン」のいずれかを含む場合のみ起動
...
**軽量パス**: 踏襲型でB〜Eのいずれのキーワードにも該当しない場合、起動するのはAgent A（codebase-analyst）のみになる。小規模な踏襲型変更（既存パターンに沿った軽微な追加・修正）はこの軽量パスで調査コストを抑える。
...
[既存アーキテクチャ制約・docs・テスト基盤 / または「スキップ（踏襲型かつアーキテクチャ変更なしのため対象外）」]

#### 依存関係（dependency-analyst）
[ライブラリ・外部API確認結果 / または「スキップ（踏襲型かつ依存関係変更なしのため対象外）」]

#### パフォーマンスリスク（performance-analyst）
[クエリパターン・キャッシュ・同時実行・スケーラビリティの設計制約 / または「スキップ（specにパフォーマンス関連の言及なし）」]

#### セキュリティ設計制約（security-analyst）
[認証・認可モデル・データ機密性・脅威ベクターの設計ガードレール / または「スキップ（specに認証・認可関連の言及なし）」]
