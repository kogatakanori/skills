...
- 見つからない場合は `tsc --noEmit` を試みる（存在しない場合はスキップ）
- test コマンドは `test` / `test:run` などを `package.json` / `Makefile` / `pyproject.toml` から検出
...
1. type-check を実行してエラーがないことを確認する（エラーがあればテストコードを修正）
2. test を実行して RED になることを最終確認する（誤って GREEN の場合はテストを修正）
3. エラー・予期しない通過がなければ commit する

### ⑨ステップ（implタスクのcommit前）

1. type-check を実行してエラーがないことを確認する（エラーがあれば実装コードを修正）
2. test を実行して全テストが GREEN になることを最終確認する
3. 全てパスすれば commit する
